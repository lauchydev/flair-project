...
export default function ProductDetailsPage() {
    ...
    async function loadProduct() {
        const res = await fetch(
            `/api/get-product?id=${encodeURIComponent(productId)}`,
            { cache: "no-store" }
        );
    ...
    async function saveDescription() {
        ...
        try {
            ...
            if (!res.ok)
                return alert(json?.error || "Failed to update description");
            setProduct(json as Product);
    ...
    async function pickColour() {
        ...
        if (window.EyeDropper) {
            try {
                ...
                if (hex)
                    setColours((prev) => Array.from(new Set([...prev, hex])));
            } catch {}
        } else {
            ...
            input.oninput = () => {
                if (input.value)
                    setColours((prev) =>
                        Array.from(new Set([...prev, input.value]))
                    );
            };
    ...
    async function deleteCurrentImage() {
        ...
        if (!id) {
            alert(
                "This image has no id. Ensure your product query returns image { id }."
            );
            return;
        }
        ...
        try {
            console.log("[deleteCurrentImage] sending", {
                apiPath,
                imageId: id,
            });
            ...
            console.log("[deleteCurrentImage] response", {
                status,
                statusText,
                payload,
            });
            ...
            if (!res.ok) {
                const msg =
                    ...
                    payload?.raw?.data?.productDeleteMedia?.userErrors?.[0]
                        ?.message ||
                    payload?.raw?.data?.productImageDelete?.userErrors?.[0]
                        ?.message ||
                    payload?.body ||
                        ...
                        <div className="flex flex-col gap-2 mb-4">
                            <label className="inline-flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    checked={customImage}
                                    onChange={(e) =>
                                        setCustomImage(e.target.checked)
                                    }
                                />
                                {customImage && (
                                    <div>
                            ...
                            <label className="inline-flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    checked={customText}
                                    onChange={(e) =>
                                        setCustomText(e.target.checked)
                                    }
                                />
                                {customText && (
                                    <div>
                            ...
                            <label className="inline-flex items-center gap-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setCustomColours(e.target.checked)
                                    }
                                />
                                {customColours && (
                                    <div>
