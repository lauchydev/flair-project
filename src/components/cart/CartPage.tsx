...
import {
    extractCustomPricing,
    calculateCustomCartTotal,
    type CustomPricedCartItem,
} from "@/lib/shopify/custom-pricing";
...
export default function CartPage() {
    ...
    const [customPricedItems, setCustomPricedItems] = useState<
        CustomPricedCartItem[]
    >([]);
    ...
    const loadCart = async () => {
        try {
            ...
            // Process custom pricing for all items
            const processedItems: CustomPricedCartItem[] =
                cartData.lines.edges.map(({ node }) => ({
                    ...node,
                    customPricing: extractCustomPricing(node),
                }));
            setCustomPricedItems(processedItems);
    ...
    const generatePreviews = async (cartData: Cart) => {
        const previewPromises = cartData.lines.edges.map(async ({ node }) => {
            ...
            const customization =
                shopifyCart.getCustomizationFromCartItem(node);
            ...
            try {
                ...
                console.error(
                    "Failed to generate preview for item:",
                    node.id,
                    error
                );
                return null;
    ...
    const handleRemoveItem = async (itemId: string) => {
        ...
        try {
            ...
            const updatedCart = await shopifyCart.removeFromCart(
                cartId,
                itemId
            );
            setCart(updatedCart);
            ...
            const processedItems: CustomPricedCartItem[] =
                updatedCart.lines.edges.map(({ node }) => ({
                    ...node,
                    customPricing: extractCustomPricing(node),
                }));
            setCustomPricedItems(processedItems);
    ...
    const getTotalPrice = () => {
        if (!cart || customPricedItems.length === 0) return "$0.00";

        const customTotal = calculateCustomCartTotal(customPricedItems);
        return `$${customTotal.total.toFixed(2)} ${customTotal.currencyCode}`;
    };
                                                ...
                                                {item.customPricing
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-600">
                                                            Base: $
                                                            {item.customPricing.basePrice.toFixed(
                                                                2
                                                            )}
                                                            {item.customPricing
                                                                .textPrice >
                                                                0 &&
                                                                ` + $${item.customPricing.textPrice.toFixed(
                                                                    2
                                                                )} text`}
                                                            {item.customPricing
                                                                .imagePrice >
                                                                0 &&
                                                                ` + $${item.customPricing.imagePrice.toFixed(
                                                                    2
                                                                )} image`}
                                                            {item.customPricing
                                                                .colorPrice >
                                                                0 &&
                                                                ` + $${item.customPricing.colorPrice.toFixed(
                                                                    2
                                                                )} color`}
                                                        </p>
                                                    </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="rounded-3xl border-4 border-black bg-white p-6 shadow-xl sticky top-6">
                                    ...
                                    return (
                                        <>
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-semibold">
                                                    $
                                                    {customTotal.subtotal.toFixed(
                                                        2
                                                    )}{" "}
                                                    {customTotal.currencyCode}
                                                </span>
                                            </div>
                                            {customPricedItems.some(
                                                (item) =>
                                                    item.customPricing
                                                        ?.hasCustomPricing
                                            ) && (
                            ...
                            <p className="mt-3 text-xs text-gray-600 text-center">
                                You&apos;ll be redirected to Shopify&apos;s
                                secure checkout
                            </p>
                        </div>
                    </div>
