...
export default function ProductCustomizer({ product }: { product: any }) {
	...
	const viewState = useViewCustomizations(config.areaCenters[config.availableViews[0] || "front"]);
	...
	const handleAddToCart = () => cart.handleAddToCart(
		config.selectedVariantId, quantity, config.selectedColor, viewState.selectedView,
		viewState.viewCustomizations, config.selectedVariant, viewState.customText, viewState.activeImageUrl
	);

	const updateViewText = (field: string, value: string) => 
		viewState.setViewCustomizations(prev => ({
			...prev,
			[viewState.selectedView]: { ...prev[viewState.selectedView], [field]: value }
		}));
			...
			<div className="container mx-auto px-4">
				<div className="mb-6 flex items-center justify-between">
					<div>
						...
						<p className="text-sm text-gray-700 font-semibold">Configure your design and add to cart</p>
					</div>
				</div>
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
					<ControlsPanel
						...
						onSelectedOptionsChange={(name, value) => config.setSelectedOptions(prev => ({ ...prev, [name]: value }))}
						enableCustomColor={config.enableCustomColor}
						enableCustomText={config.enableCustomText}
						enableCustomImage={config.enableCustomImage}
						availableColours={config.availableColours}
						...
						currentViewCustomization={viewState.currentViewCustomization}
						selectedView={viewState.selectedView}
						currentCenter={config.areaCenters[viewState.selectedView]}
						onViewCustomizationsChange={viewState.setViewCustomizations}
						imagePrice={config.imagePrice}
						...
						onFontChange={(font) => updateViewText('textFont', font)}
						onColorChange={(color) => updateViewText('textColor', color)}
						quantity={quantity}
