				...
				<ImageUpload
					...
					onAdd={(urls: string[]) =>
						onViewCustomizationsChange((prev) => {
							...
							const nextActive =
								view.activeImageIndex !== null ? view.activeImageIndex : 0;
							return {
								...prev,
								[selectedView]: {
									...view,
					...
					onRemove={(index: number) =>
						onViewCustomizationsChange((prev) => {
							...
							let nextActive: number | null = view.activeImageIndex;
							if (nextActive === index) {
								nextActive = imgs.length ? 0 : null;
							} else if (nextActive !== null && index < nextActive) {
							...
							return {
								...
								[selectedView]: {
									...
									imagePos: overlay
										...
										: view.imagePos,
									imageWidthPercent:
										overlay?.widthPercent ?? view.imageWidthPercent,
									imageHeightPercent:
										overlay?.heightPercent ?? view.imageHeightPercent,
									imageAngleDeg: overlay?.angleDeg ?? view.imageAngleDeg,
								},
							};
						})
					}
